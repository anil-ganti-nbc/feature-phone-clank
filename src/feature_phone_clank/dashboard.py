...
from feature_phone_clank.collector_ui import (
    CSS as UI_CSS, DESIGN_SYSTEM_VERSION,
    badge as _ui_badge, empty as _ui_empty,
)

# Feature Phone Clank domain accent (the only visual token this Clank overrides).
ACCENT, ACCENT_SOFT = "#c084fc", "#2a1b3d"
...
def table(h, rows, title, detail):
    ...
    head = "".join(f"<th>{e(x)}</th>" for x in h)
...
def render(db, controller=None, qc_db=None):
    ...
    # STD-UI-COM-009: `status` already separates ok / blocked_zero_result /
    # out_of_scope, but two runs that both land on `failed` for materially
    # different reasons (a fetch error vs a parser blowing up) rendered
    # identically -- the backend keeps the distinction in run_errors, per run.
    # Surface it inline next to the status it qualifies.
    runrows=[(e(x['source_key']),e(x['started_at']),e(x['status']),e(x['products_observed'] or '—'),e('; '.join(_run_errors.get(x['id'],[])) or '—')) for x in runs]
    ...
    css = ("<style>" + UI_CSS + chr(10)
           + ":root{--accent:" + ACCENT + ";--accent-soft:" + ACCENT_SOFT + ";}</style>")
    js = '''<script>
